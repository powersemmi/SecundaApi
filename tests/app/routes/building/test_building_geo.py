...
async def test_building_geo_endpoints_require_api_key_negative(
...
async def test_list_buildings_by_geo_radius_positive(
...
async def test_list_buildings_by_geo_bbox_positive(
    api_client, api_headers, assert_json_list, build_url
):
    response = await api_client.get(
        build_url("/building/geo"),
...
async def test_list_buildings_by_geo_invalid_params_negative(
